#include <stdio.h>
  int main() {
        ...
        int data[9], index, sign, player, flag, i, k, j,s;
            printf("\t\t\t XO GAME\n\tplayer 1 =>'X'\tplayer 2 =>'O'");
            printf("\n No worries screen refreshes every 2 sec.\n");
        ...
        while (count < 9) {
                ...
                scanf("%d", &index);
                if (index < 1 || index > 9) {
                        printf("Index from 1 to 9 is only allowed !!/\n");
                        sleep(2);
                        continue;
                }
                if (data[index - 1] == 'X' || data[index - 1] == 'O') {
                        printf("OOPS!! Position Already occupied.Try another Position\n");
                        sleep(2);
                        continue;
                }
                data[index - 1] = sign;
